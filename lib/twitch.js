...
// Categoría/juego que tienes configurado actualmente en el canal (aunque
// no estés en vivo, Twitch guarda la última categoría seteada).
export async function getChannelInfo(broadcasterId, clientId, clientSecret) {
  const data = await twitchFetch(`channels?broadcaster_id=${broadcasterId}`, clientId, clientSecret);
  const info = data.data?.[0];
  return {
    gameName: info?.game_name || null,
    title: info?.title || null,
  };
}

// Los clips más vistos del canal en los últimos N días (por defecto los
// últimos 30). Get Clips de Twitch ya los devuelve ordenados por vistas,
// de mayor a menor.
export async function getTopClips(broadcasterId, clientId, clientSecret, { withinDays = 30, first = 4 } = {}) {
  const startedAt = new Date(Date.now() - withinDays * 24 * 60 * 60 * 1000).toISOString();
  const data = await twitchFetch(
    `clips?broadcaster_id=${broadcasterId}&first=${first}&started_at=${startedAt}`,
    clientId,
    clientSecret
  );
  return data.data || [];
...
// El último video/VOD subido al canal (type=archive = transmisiones
// pasadas guardadas; excluye highlights y uploads sueltos). Ordenado por
// fecha, el más reciente primero.
export async function getLatestVideo(broadcasterId, clientId, clientSecret) {
  const data = await twitchFetch(
    `videos?user_id=${broadcasterId}&type=archive&sort=time&first=1`,
    clientId,
    clientSecret
  );
